...
- `packages/bare-css` — the website's CSS system (tokens, bare element styles). Read `website/DESIGN.md` before changing it.
- `website/` — demo playground that consumes castro. Site copy and voice: `website/README.md`; visual design: `website/DESIGN.md`. `website/tsconfig.json` is the canonical tsconfig; `packages/create-castro/template/tsconfig.json` shares its `compilerOptions` apart from `paths`, which points at root-level `pages/`/`layouts/` instead of `src/`; it also carries no `exclude`. `tests/scaffolder-template.test.js` holds that to it.
