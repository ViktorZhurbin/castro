...
- `website/` — demo playground that consumes castro. Its CSS system (tokens, bare element styles) lives in `website/src/styles/`. Site copy and voice: `website/README.md`; visual design: `website/DESIGN.md`. `website/tsconfig.json` is the canonical tsconfig; `packages/create-castro/template/tsconfig.json` shares its `compilerOptions` apart from `paths`, which points at root-level `pages/`/`layouts/` instead of `src/`; it also carries no `exclude`. `tests/scaffolder-template.test.js` holds that to it.
