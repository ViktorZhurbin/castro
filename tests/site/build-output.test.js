...
// `_` — a private directory (pages/_drafts/) and a private file at the top
// level (pages/_partial.tsx). No other test in this suite asserts what dist/
// must not contain, so a regression here would otherwise go unnoticed.
describe("private paths (`_` prefix)", () => {
  ...
  // Mirrors scanPages()'s own segment check against actual build output,
  // catching a leak under any filename — not just the two fixtures above.
...
describe("hydration directives", () => {
  describe("comrade:visible", () => {
    test("has island wrapper", async () => {
    ...
    test("has island runtime", async () => {
      const html = await readHtml("comrade-visible.html");
      expect(html).toContain("castro-island.js");
    });

    test("has island JS bundle reference", async () => {
      const html = await readHtml("comrade-visible.html");
      expect(html).toContain('import="/');
    });

    test("has SSR content", async () => {
      const html = await readHtml("comrade-visible.html");
      expect(html).toContain("Count:");
    });

    test("has island CSS", async () => {
      const html = await readHtml("comrade-visible.html");
  ...
  describe("comrade:eager", () => {
    test("has correct directive", async () => {
...
describe("island props serialization", () => {
  ...
  // renderMarker() must pass all three — narrowing the guard to reject either
  // one throws on `{cond ? <X/> : null}`, caught only by this build failing.
  // Asserting the whole serialized object (not just the surviving prop) is
  // what would catch a leaked `children` key showing up in these braces.
...
describe("layouts", () => {
  ...
  // (writeHtmlPage.js).
  describe("shell-less layout (no <head>/<body> to anchor to)", () => {
    test("page still has DOCTYPE and content", async () => {
      const html = await readHtml("bare-layout.html");
      expect(html).toContain("<!DOCTYPE html>");
      expect(html).toContain("<h1>Bare Layout Test</h1>");
    });

    test("page still gets the island runtime and import map", async () => {
      const html = await readHtml("bare-layout.html");
      expect(html).toContain("castro-island.js");
      expect(html).toContain('type="importmap"');
    });

    test("page still gets inline island CSS", async () => {
      const html = await readHtml("bare-layout.html");
      expect(html).toContain("<style>");
      expect(html).toContain("color: red");
    });

    test("page still gets its page CSS link", async () => {
      const html = await readHtml("bare-layout.html");
  ...
  describe("nested layouts (layout id includes the directory)", () => {
...
describe("islands on a page", () => {
  ...
  // to /islands/. Badge, not Counter, because the vendored-deps test below
  // globs `islands/Counter.island-*.js` and `*` doesn't cross a directory separator.
...
// the marker, the component would bundle inline instead — "Count:" would
// still render, but without the <castro-island> wrapper this asserts on.
describe("island import resolution (extensionless + tsconfig alias)", () => {
...
describe("import map and vendored deps", () => {
  ...
  // @preact/signals reaches the import map through castro.config.ts's
  // clientDependencies, not PREACT_CLIENT_DEPS.
...
// Every test above stops at the marker the build emits and never executes
// castro-island.js — that's covered separately in core/src/islands/
// castroIsland.test.js, against a DOM. This block catches the seam between
// them: a renamed attribute on either side, invisible to both suites alone.
// Both sides are read out of dist/ (real markup, the shipped runtime file),
// not core's source, so a stale docblock can't paper over a real mismatch.
describe("hydration seam (build output ↔ shipped runtime)", () => {
