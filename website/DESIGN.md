...
The visual system lives in the `@vktrz/css` package (`packages/css/src/`), pulled in via `import "@vktrz/css/style.css"` in `PageShell` — castro bundles it into the page CSS. The package is organized as:
...
`@vktrz/css` uses two naming layers. Work with the role variables in component CSS; reference the raw materials only when defining new theme rules.
...
**Display** (`var(--font-display)`, Bebas Neue): headings, uppercase labels, the brand mark. Global `letter-spacing: 0.05em` is applied in `@vktrz/css` to all heading elements. Don't remove it.

**Body** (Barlow, `font-weight: 500`): set via `--font-family` and `--font-weight`. Default for all other text.

**Font sizes**: Use `var(--text-xs)` through `var(--text-2xl)` for `font-size` declarations. Raw rem values belong in `@vktrz/css` only, where the scale is defined.

**Heading scale**: `h1` and `h2` override `--font-size` in `@vktrz/css`, with a responsive bump at 768px. `h1` uses `--h1-color: var(--primary)` so top-level headings are automatically crimson/gold.

## Border System

The constructivist visual weight comes from border geometry, not shadows. Four weights are defined in `@vktrz/css`:
...
**Zero border radius** — `--border-radius: 0rem` in `@vktrz/css` applies globally. Don't add `border-radius` in component CSS.
...
**Breakpoints** (documented in `@vktrz/css`):
...
Buttons are pre-styled in `@vktrz/css` (`elements.css`) — a bare `<button>` already carries the base look. Anchors that should read as buttons take `role="button"`.
