...
Accent on text is reserved for display-size figures (section numbers, the `1,350` tally, the Five-Year Plan readout, the docs page title). Body text is never colored; links are ink with an accent underline. Code is the second exception alongside `--code-background`: the docs syntax theme (`src/components/highlight/syntaxHighlight.css`) prints in two plates — keywords, numbers, booleans and the command word of a shell line in the accent; strings, JSX tags, called functions, shell arguments and frontmatter keys in a blue; imported names and markdown headings in a darker blue — and sets comments in a muted grey, the one place on the site that has one. `::highlight()` cannot change weight or style, so hue is the lever a code block has; the one exception is the markdown heading, whose bold is drawn with `text-shadow`. Those hues are declared in that stylesheet, not in `tokens.css`: they are not materials the rest of the site may reach for, and a page or component using one is a bug.
