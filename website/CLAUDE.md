...
This is a demo site that consumes Castro. Global styling comes from `import "../styles/index.css"` in `PageShell`. Each component/page also has its own co-located CSS file that consumes the tokens in `src/styles/tokens.css` (`--primary`, `--spacing-*`, `--text-*`, `--border-*`).
...
**Read `DESIGN.md` before any visual change** — site CSS, `src/styles/`, or porting a Claude Design mockup. It holds the principles, tokens, type, rules, and where the design source lives. The tokens and pre-styled bare elements live in `src/styles/`; the site defines only component-specific CSS on top.
